...
interface ServiceOptions<T, Params extends Record<string, unknown>, C extends Cursor> {
  /**
...
export function createServiceProvider(client: HttpClient, defaultMapping?: MappingConfig) {
  ...
  >(baseUrl: string, keys: QueryKeys<Params>, options?: ServiceOptions<T, Params, C>) {
    ...
    // Resolve mapping: merge service-specific > factory-default
    ...
    const api = {
       ...
       */
      detail:
        options?.overrides?.detail ??
        ((id: Id): Promise<T> =>
          client.get<unknown>(`${baseUrl}/${id}`).then((res) => responseMapper.mapItem<T>(res))),
      ...
      create:
        ...
        ((data: Partial<T>): Promise<T> =>
          client.post<unknown>(baseUrl, data).then((res) => responseMapper.mapItem<T>(res))),
      ...
      update:
        ...
        ((id: Id, data: Partial<T>): Promise<T> =>
          client
            .put<unknown>(`${baseUrl}/${id}`, data)
            .then((res) => responseMapper.mapItem<T>(res))),
